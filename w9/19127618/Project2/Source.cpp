...
vector<myString> myString::Split(vector<char> a, bool flag)
{
	...
	for (int i = 0; i < Data.length(); i++)
		if (flag == 1 && checkspace(a, Data[i], Data[i + 1]))continue;
...
map<string, int> myString::compact(vector<myString> split)
{
	map<string, int> mymap;

	for (int i = 0; i < split.size() - 1; i++)
		mymap.insert(pair<string, int>(split.at(i).Data, counting(split, split.at(i))));
	return mymap;
}
int myString::counting(vector<myString> vms, myString ms)
{
	int j = 0; int k = 0;
	for (auto i = vms.begin(); i != vms.end(); i++) {
		if (ms.Data == vms.at(j).Data)
			k++;
		j++;
	}
	return k;
